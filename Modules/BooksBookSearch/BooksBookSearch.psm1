...
function Get-ComicInfo {
    ...
    Begin {
        ...
        [void]$uriBuiler.Append("&hits=1&elements=Items&formatVersion=2&booksGenreId=001001&title=")
    ...
    Process {
        try {
            sleep -Seconds 1
            $encodedTitle = [HttpUtility]::UrlEncode($title.Replace(" ", ""))
            $response = curl -Uri ($uri + $encodedTitle)
            $result = $response.Content | ConvertFrom-Json
            if ($result.Items.Count -eq 0) {
                #何も取得できなかったデータに対してそのメッセージを格納する。
                $catchModel = [PSCustomObject]@{
                    title = "404"
                    titleKana = $Title
                }
                return $catchModel
